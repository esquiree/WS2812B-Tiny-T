...
typedef struct cRGBW { uint8_t g; uint8_t r; uint8_t b; uint8_t w;}cRGBW; 
typedef struct cHSV{ uint8_t h; uint8_t s; uint8_t v;} cHSV;
 ...
 *
 * Precision is lost since floating point operations are not used. 
 * Round trip color conversion is not guaranteed. 
 */
